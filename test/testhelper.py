...
def broadcast_message( src_uid, *parts ):
    if len(parts) == 1 and isinstance(parts[0], (list, tuple)):
        parts = parts[0]
    for n in nodes.itervalues():
        if src_uid != n.node_uid or n.recv_self:
            n.dispatch_message( None, parts )
            

def unicast_message( src_uid, dst_uid, *parts ):
    if len(parts) == 1 and isinstance(parts[0], (list, tuple)):
        parts = parts[0]
    if dst_uid in nodes:
        nodes[dst_uid].dispatch_message( src_uid, parts )
...
class NetworkNode (object):
    ...
    def broadcast_message(self, *parts):
        ...
        if isinstance(parts, tuple):
            parts = list(parts)
        broadcast_message(self.node_uid, parts)
    ...
    def unicast_message(self, node_uid, *parts):
        ...
        if isinstance(parts, tuple):
            parts = list(parts)
        unicast_message(self.node_uid, node_uid, parts)
